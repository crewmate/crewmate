def teambox_data_import(make_data)
  if ENV['CREWMATE_DATA']
    #begin
      ...
      CrewmateData.import_from_file(ENV['CREWMATE_DATA'], object_maps, {:create_users => make_data, 
                                                                      :create_organizations => make_data,
                                                                      :format => ENV['CREWMATE_DATA_FORMAT']})
    #rescue Exception => e
    #  puts e
    #end
  else
    puts "Please specify your crewmate dump via CREWMATE_DATA"
  end
end
...
namespace :data do
  ...
  task :export => :environment do
    begin
      CrewmateData.export_to_file(Project.all, User.all, Organization.all, "export-#{Time.now.to_s}.json")
    rescue Exception => e
      puts e
    end
  end
